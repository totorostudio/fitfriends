...
export function MainPage(): JSX.Element {
  ...
  const relatedTrainings: Training[] = data?.trainings || []; // TODO в бэкенде сделать вывод релевантных тренировок

  useEffect(() => {
    if (userInfo.data && userInfo.data.id) {
      console.log('Запускаю диспатч')
      dispatch(fetchTrainingsAction({}));
    }
  }, [dispatch, userInfo.data, userInfo.data?.id]);
